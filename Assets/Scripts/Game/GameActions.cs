...
public class GameActions : MonoBehaviour
{
    ...
    public void DoubleIt()
    {
        if (_canTake)
        {
            ...
            if (_gotJoker == false) StartCoroutine(PauseWait());
        }
    }
    ...
    private IEnumerator PauseWait()
    {
        ...
        while (_botScore < 17 && _gotJoker == false)
        {
            if (_botScore < 17 && _gotJoker == false)
                GiveBotCard();
            else
                break;
            yield return new WaitForSeconds(1.5f);
        ...
        if (_gotJoker == false) CheckStatus();
    }
